...
```bash
sudo pacman -S base base-devel linux linux-firmware amd-ucode dhcpcd iwd curl unzip zsh vim xorg xorg-xinit i3 dex ttf-dejavu man-pages man-db dmenu polkit xdg-utils nodejs npm rustup python git rsync fasd fzf tmux zsh-syntax-highlighting openssh keychain pass pinentry ruby btop terminator cronie zsh-autosuggestions nmap ufw
...
pikaur -S i3lock-color brave-bin enpass-bin sublime-text-4 pinta claude-code xidlehook pipes.sh lsu-git psn-git
```
